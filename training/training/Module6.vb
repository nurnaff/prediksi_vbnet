...
Module Module6
    ...
    Sub main()
        koneksi()
        For i = 0 To 60
            cm = New OdbcCommand("select x1 from trainemas where no=" & i, kon)
            x1(i) = cm.ExecuteScalar
        Next
        For i = 0 To 60
            cm = New OdbcCommand("select x2 from trainemas where no=" & i, kon)
            x2(i) = cm.ExecuteScalar
        Next
        For i = 0 To 60
            cm = New OdbcCommand("select y from trainemas where no=" & i, kon)
        ...
        Do While Not (kondisi = True)
            For i = 0 To 60
                ...
                WriteLine("w31=" & w31)
                WriteLine("w41=" & w41)
                If ((tse < 0.00005) And (i Mod 59 = 0)) Then
                    kondisi = True
                End If
            Next
            epoch += 1
