...
Module Module3
    ...
    Sub main()
        koneksi()
        For i = 0 To 3
            cm = New OdbcCommand("select x1 from uji where no=" & i, kon)
            x1(i) = cm.ExecuteScalar
        Next
        For i = 0 To 3
            cm = New OdbcCommand("select x2 from uji where no=" & i, kon)
            x2(i) = cm.ExecuteScalar
        Next
        For i = 0 To 3
            cm = New OdbcCommand("select y from uji where no=" & i, kon)
        ...
        Do While Not (kondisi = True)
            For i = 0 To 3
                ...
                z1 = (2.0 / (1.0 + (Math.Exp(-z_in1)))) - 1.0
                z2 = (2.0 / (1.0 + (Math.Exp(-z_in2)))) - 1.0
                z3 = (2.0 / (1.0 + (Math.Exp(-z_in3)))) - 1.0
                z4 = (2.0 / (1.0 + (Math.Exp(-z_in4)))) - 1.0
                y_in1 = w01 + (z1 * w11) + (z2 * w21) + (z3 * w31) + (z4 * w41)
                y1 = (2.0 / (1.0 + (Math.Exp(-y_in1)))) - 1.0
                salah = t(i) - y1
                tse += (salah ^ 2)
                dy1 = salah * ((2.0 / (1.0 + (Math.Exp(-y_in1)))) - 1.0)
                Dw11 = alpha * dy1 * z1
                Dw21 = alpha * dy1 * z2
                Dw31 = alpha * dy1 * z3
                Dw41 = alpha * dy1 * z4
                ...
                dz1 = d_in1 * 0.5 * (1.0 + z1) * (1.0 - z1)
                dz2 = d_in2 * 0.5 * (1.0 + z2) * (1.0 - z2)
                dz3 = d_in3 * 0.5 * (1.0 + z3) * (1.0 - z3)
                dz4 = d_in4 * 0.5 * (1.0 + z4) * (1.0 - z4)
